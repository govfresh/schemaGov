...
A government publishing to one shared format, and a vendor or developer building against it,
both do less one-off work. The same data can drive a website, a mobile app, an open data
portal, a search listing, or an AI chatbot, without a separate integration for each.

### No vendor lock-in

The format is open and public domain, so no single vendor controls it. A government can
change providers, or run several systems side by side, without its published data becoming
unusable.
